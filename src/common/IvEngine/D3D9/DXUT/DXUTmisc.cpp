...
HRESULT DXUTGetDXSDKMediaPathCch( WCHAR* strDest, int cchDest )
{
    ...
    StringCchCopy( strDest, cchDest, TEXT("") );
    ...
    lResult = RegQueryValueEx( hKey, L"DX9D4SDK Samples Path", NULL,
    ...
    if( lstrlen(strDest) + lstrlen(strMedia) < cchDest )
        StringCchCat( strDest, cchDest, strMedia );
    else
        return E_INVALIDARG;
                                     ...
                                     WCHAR* strExePath, WCHAR* strExeName, LPCWSTR strMediaDir )
{
    ...
    StringCchPrintf( strSearchPath, cchSearch, L"%s%s", strMediaDir, strLeaf ); 
    if( GetFileAttributes( strSearchPath ) != 0xFFFFFFFF )
        return true;
