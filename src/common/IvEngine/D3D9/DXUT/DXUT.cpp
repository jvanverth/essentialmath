...
void DXUTUpdateStaticFrameStats()
{
    ...
    WCHAR* pstrStaticFrameStats = GetDXUTState().GetStaticFrameStats();
    StringCchPrintf( pstrStaticFrameStats, 256, L"%%.02f fps (%dx%d), %s%s%s", 
...
void DXUTDisplayErrorMessage( HRESULT hr )
{
    ...
    switch( hr )
    {
        ...
        case DXUTERR_SWITCHEDTOREF:          nExitCode = 0; StringCchCopy( strBuffer, 512, L"Switching to the reference rasterizer,\na software device that implements the entire\nDirect3D feature set, but runs very slowly." ); break;
